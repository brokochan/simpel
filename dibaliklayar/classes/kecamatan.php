...
class kecamatan extends DbTable
{
	...
	public function __construct()
	{
		...
		// id_kecamatan
		$this->id_kecamatan = new DbField('kecamatan', 'kecamatan', 'x_id_kecamatan', 'id_kecamatan', '`id_kecamatan`', '`id_kecamatan`', 3, 11, -1, FALSE, '`id_kecamatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_kecamatan->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_kecamatan->IsPrimaryKey = TRUE; // Primary key field
		...
		// kecamatan
		$this->kecamatan = new DbField('kecamatan', 'kecamatan', 'x_kecamatan', 'kecamatan', '`kecamatan`', '`kecamatan`', 200, 32, -1, FALSE, '`kecamatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// kodepos
		$this->kodepos = new DbField('kecamatan', 'kecamatan', 'x_kodepos', 'kodepos', '`kodepos`', '`kodepos`', 200, 32, -1, FALSE, '`kodepos`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			if (array_key_exists('id_kecamatan', $rs))
				AddFilter($where, QuotedName('id_kecamatan', $this->Dbid) . '=' . QuotedValue($rs['id_kecamatan'], $this->id_kecamatan->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_kecamatan', $row) ? $row['id_kecamatan'] : NULL;
		else
			$val = $this->id_kecamatan->OldValue !== NULL ? $this->id_kecamatan->OldValue : $this->id_kecamatan->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_kecamatan@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		$json = "";
		$json .= "id_kecamatan:" . JsonEncode($this->id_kecamatan->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_kecamatan->CurrentValue != NULL) {
			$url .= "id_kecamatan=" . urlencode($this->id_kecamatan->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_kecamatan") !== NULL)
				$arKeys[] = Param("id_kecamatan");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				...
				$this->id_kecamatan->OldValue = $key;
